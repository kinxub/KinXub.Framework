...
namespace KinXub.Framework.RepositoryDal.Base
{
    public static partial class SqlMapperExtensions
    {
        ...
        public static async Task<bool> UpdateAsync<T>(this IDbConnection connection, object entityToUpdate, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            var proxy = entityToUpdate as IProxy;
            if (proxy != null)
            {
            ...
            var objtype = entityToUpdate.GetType();
            var allProperties = objtype.GetProperties();
            keyProperties.AddRange(explicitKeyProperties);
            var computedProperties = ComputedPropertiesCache(objtype);

            List<PropertyInfo> nonIdProps = new List<PropertyInfo>();
            foreach (var key in keyProperties)
            {
                foreach (var item in allProperties)
                {
                    if (key.Name != item.Name) nonIdProps.Add(item);
                }
            }
            if (0 == nonIdProps.Count)
                throw new ArgumentException("Entity must have at least one data");
